...
class PostsController extends Controller
{
    ...
    public function sort(Request $request){
        ...
        if(count($item) != 0){
            foreach($item as $key => $value){
                $item2 = Post::where("id",$value)->first();
                if($item2 != null){
                    $item2->sort = $key;
                    $item2->update();
                }
    ...
    function get_data(Request $request)
    {
        ...
        $totalData = Post::where(function ($q) use($type){
                if($type){
                    $q->where("type",$type);
                }
            })->count();
        ...
        if (!empty($posts)) {
            foreach ($posts as $post) {
                ...
                $add = '<label class="custom-switch"> <input type="checkbox" data-id='. $post->id .' name="custom-switch-checkbox" class="btn_featured custom-switch-input" '.$check.'> <span class="custom-switch-indicator"></span> <span class="custom-switch-description">'.$active_or_no.'</span> </label>';

                $nestedData['featured'] =$add;

                $type = parent::CurrentLangShow()->Type_Blog;;
                if ($post->type == 2) {
                    $type = parent::CurrentLangShow()->Type_Header;
                } else if ($post->type == 3) {
    ...
    public function post_data(Request $request)
    {
        ...
        if ($validation->fails()) {
            ...
            if ($edit != null) {
                ...
                $Post->update();
                return response()->json(['success' => __('language.msg.m'), 'dashboard' => '1', 'redirect' => route('dashboard_posts.add_edit',['id'=>$Post->id])]);
            } else {
                $Post = new Post();
                $Post->name = Input::get('name');
                $Post->type = Input::get('type');
                ...
                $Post->save();
                return response()->json(['success' => __('language.msg.s'), 'dashboard' => '1', 'redirect' => route('dashboard_posts.add_edit',['id'=>$Post->id])]);
            }
        }
    }
