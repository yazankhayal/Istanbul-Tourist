    ...
    function get_data(Request $request)
    {
        ...
        if (!empty($posts)) {
            foreach ($posts as $post) {
                ...
                $sele_r = __('table.other_lang');
                if ($post->select == 1) {
                    $sele_r = __('table.primary');
                }
        ...
        if ($validation->fails()) {
            ...
            if ($edit != null) {
                DB::transaction(function () {
                    ...
                    $Language->dir = Input::get('dir');
                    if (Input::get('select') == "on") {
                        $Language->select = true;
                        Session::put('local', Input::get('dir'));
                        app()->setLocale(Input::get('dir'));
                        $get_other = Language::where('select', '=', '1')->first();
                        $get_other->select = 0;
                        $get_other->update();
                        $env_update = [
                            'APP_LOCAL' => Input::get('dir'),
                        ];
