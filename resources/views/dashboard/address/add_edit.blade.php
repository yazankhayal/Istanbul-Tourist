...
@section('create_btn'){{route('dashboard_address.index')}}@endsection
                        ...
                        <div class="tab-content" id="myTabContent">
                            <div class="tab-pane fade show active" id="home" role="tabpanel"
                                ...
                                <form class="ajaxForm users" enctype="multipart/form-data" data-name="users"
                                      action="{{route('dashboard_address.post_data')}}" method="post">
                                    {{csrf_field()}}
                                    ...
                                    <div class="modal-footer">
                                        ...
                                        <a href="{{route('dashboard_address.index')}}" class="btn btn-default">
                                            {{$lang->Close}}
                                        </a>
                                ...
                                @foreach($langauges->where('dir','!=',$select_lan_choice->dir) as $lang222)
                                    <div class="tab-pane fade tab_{{$lang222->id}}" id="{{$lang222->name}}"
                                        ...
                                        <form class="ajaxForm translate" data-name="translate"
                                              action="{{route('dashboard_address_translate.post_data')}}" method="post">
                                            <div class="modal-body row">
                                                {{csrf_field()}}
                                                <input id="id_current_{{$lang222->id}}" name="id" type="hidden">
                                                <input id="hp_contents_id_{{$lang222->id}}" name="hp_contents_id"
                                                       type="hidden">
                                                <input id="language_id_{{$lang222->id}}" name="language_id"
                                                       type="hidden"
                                                       value="{{$lang222->id}}">
                                                ...
                                                <div class="form-group col-md-12">
                                                    <div class="form-group col-md-12">
                                                        <label for="summary">{{$lang->Summary}}</label>
                                                        <textarea rows="4" class="cls sumernote  form-control"
                                                                  name="summary"
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            if (isNaN(last_part) == false) {
                ...
                var ed = '<div class="col-md-6 col-lg-4 col-xl-4"><div class="why_chose_us"><div class="icon"><img alt="icon"></div><div class="details"><h4>Trusted By Thousands</h4><p>Aliquam dictum elit vitae mauris facilisis at dictum urna dignissim donec vel lectus vel felis.</p></div></div></div>';
            ...
            $(document).on("click", ".get_content_language", function () {
                ...
                var id = $("#id").val();
                $.ajax({
                    url: "{{route('dashboard_address_translate.get_data_by_id')}}",
                    method: "get",
                    data: {
                        "id": id,
        ...
        var sumaray = function (x, y) {
            tinymce.get(x).setContent(y);
        }

        var Render_Data = function (id) {
            $.ajax({
                url: "{{route('dashboard_address.get_data_by_id')}}",
                method: "get",
                data: {
                    "id": id,
                },
                ...
                success: function (result) {
                    if (result.success != null) {
                        ...
                        window.location.href = "{{route('dashboard_address.index')}}";
                    }
                }
            });
        };
