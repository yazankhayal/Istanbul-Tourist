...
@section('create_btn'){{route('dashboard_catalogue.add_edit')}}@endsection
    ...
    <div class="modal fade" id="ModNewMore" tabindex="-1" role="dialog">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form class="ajaxForm translate" data-name="translate" action="{{route('dashboard_catalogue_translate.post_data')}}" method="post">
                    <div class="modal-body row">
                        {{csrf_field()}}
                        <input id="id_current" name="id" type="hidden">
                        <input id="catalogue_id" name="catalogue_id" type="hidden">
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $(document).on('click', '.btn_edit_lan', function () {
                ...
                $('.form-control').removeClass('border-danger');
                $.ajax({
                    url:"{{route('dashboard_catalogue_translate.get_data_by_id')}}",
                    method:"get",
                    data : {
                        "id" : id,
                    },
            ...
            $('.btn_deleted').on("click",function () {
                var id = $('#iddel').val();
                $.ajax({
                    url:"{{ route('dashboard_catalogue.deleted') }}",
                    method:"get",
                    data : {
                        "id" : id,
                    },
        ...
        var Render_Data = function () {
            datatabe = $('#data_Table').DataTable({
                ...
                "ajax":{
                    "url": "{{ route('dashboard_catalogue.get_data') }}",
                    "dataType": "json",
                    "type": "POST",
                    "data":{
                        _token: "{{csrf_token()}}",
