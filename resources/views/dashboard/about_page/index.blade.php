                    ...
                    <div class="form-group m-form__group">
                        ...
                        <div class="tab-content" id="myTabContent">
                            <div class="tab-pane fade show active" id="home" role="tabpanel"
                                ...
                                <hr>
                                <form class="ajaxForm dashboard_about_page" enctype="multipart/form-data"
                                      data-name="dashboard_about_page"
                                      action="{{route('dashboard_about_page.post_data')}}" method="post">
                                    {{csrf_field()}}
                                    ...
                                    <div class="modal-footer">
                                        ...
                                        <a href="{{route('dashboard_about_page.index')}}" class="btn btn-default">
                                            {{$lang->Close}}
                                        </a>
                                ...
                                @foreach($langauges->where('dir','!=',$select_lan_choice->dir) as $lang222)
                                    <div class="tab-pane fade tab_{{$lang222->id}}" id="{{$lang222->name}}" role="tabpanel"
                                        ...
                                        <form class="ajaxForm translate" data-name="translate"
                                              action="{{route('dashboard_about_page_translate.post_data')}}" method="post">
...
@section('js')
    ...
    <script type="text/javascript">
        ...
        var other = function(x,y){
            $.ajax({
                url: "{{route('dashboard_about_page_translate.get_data_by_id')}}",
                method: "get",
                data: {
                    "id": x,
                    "language_id": y,
                ...
                success: function (result) {
                    if (result.success != null) {
                        ...
                        var ed = '<div class="row"><div class="col-lg-8"><div class="start_partner tac-smd"><h2>Become a Real Estate Agent</h2><p>We only work with the best companies around the globe</p></div></div><div class="col-lg-4"><div class="parner_reg_btn text-right tac-smd"><a class="btn btn-thm2" href="#">Register Now</a></div></div></div>';
        ...
        var Render_Data = function () {
            $.ajax({
                url: "{{ route('dashboard_about_page.get_data_by_id') }}",
                method: "get",
                data: {},
                dataType: "json",
                success: function (result) {
                    if (result.success != null) {
                        ...
                        $(window).on('load', function () {
                            var ed = '<div class="row"><div class="col-lg-8"><div class="start_partner tac-smd"><h2>Become a Real Estate Agent</h2><p>We only work with the best companies around the globe</p></div></div><div class="col-lg-4"><div class="parner_reg_btn text-right tac-smd"><a class="btn btn-thm2" href="#">Register Now</a></div></div></div>';
                            tinymce.get('summary').setContent(ed);
                        });
                    }
                }
            });
